...
int test(char32_t ch, const char *szCharDescr)
{
	...
	sprintf_s(szUnicodeHex, 9, "%04X", (unsigned)ch);
...
int main(int argc, char* argv[])
{
...

	{
		...
		rlText::File file(u8"../test-textfiles/UTF-32 BE.txt", 0);
		if (file)
		{
			...
			printf("  Linebreak type: %s.\n", szLineBreakName[file.getLinebreakType()]);

			if (!file.save(u8"C:\\[TempDel]\\test.txt", 0, 0))
